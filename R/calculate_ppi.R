...
#' @returns An sf object of PPI estimates for site.
calculate_ppi <- function( locs, buffer, compiled_raster, progress = FALSE ){
  ...
  print(paste0( "Calculating PPI for ", nrow( locs), " ", tolower(as.vector(unique(st_geometry_type(locs)))), ifelse(nrow( locs)>1, "s")))
  calcs <-  lapply( as.list(setNames(buffer,paste0(paste('PPI', buffer, sep = "-"), "km"))), function(b){
    print(paste0("... ", b, "km buffer"))
    X <- st_buffer( st_transform( locs, raster::crs(compiled_raster)), dist = b*1000)
