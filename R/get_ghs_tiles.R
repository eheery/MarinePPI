...
#' @returns sf POLYGON object of tile areas
get_ghs_tiles <- function(study_area, 
  ...
  if(length(which( endsWith( fls, "shp") & (stringr::str_detect( fls, "tile_schema")) )) == 0){
    ...
    temp <- tempfile()
    download.file(tile_schema_URL,temp)
    unzip( temp, exdir = paste( getwd(), data_directory, sep = "/") )
    unlink(temp) 
    fls <- list.files(paste( getwd(), data_directory, sep = "/"))
    ...
    tile_schema_filename <- fls[which( endsWith( fls, "shp") & (stringr::str_detect( fls, "tile_schema")) )] }
  tile_schema <- st_read( paste( getwd(), data_directory, tile_schema_filename, sep = "/"),quiet=TRUE)
 
  # Convert to mollweide
  g_moll <- st_transform( study_area, crs = st_crs(tile_schema))
  
  # Filter out intersecting tiles
  tl <- st_filter( tile_schema , g_moll, .predicate = st_intersects)
  ...
  if(any(tl_report$AlreadyAccessed == FALSE)){
    id <- with(subset(tl_report, AlreadyAccessed == FALSE ), SelectedTiles)
