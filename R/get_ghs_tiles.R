...
#' @return list of three objects: (1) a data frame showing the tiles that were downloaded, (2) an sf POLYGON object of tile areas, and (3) an sf object of the study area grid, converted to Mollweide (the coordinate reference system of the imported rasters)
#' @export
get_ghs_tiles <- function(study_area, 
  ...
  if(length(which( endsWith( fls, "shp") & (stringr::str_detect( fls, "tile_schema")) )) == 0){
    ...
    tile_schema_filename <- fls[which( endsWith( fls, "shp") & (stringr::str_detect( fls, "tile_schema")) )] }
  tile_schema <- sf::st_read( paste( data_directory, tile_schema_filename, sep = "/"),quiet=TRUE)
  ...
  accesseddatetime <- sapply( tl_report$Filename, function(x){ 
    as.character(file.info(paste( data_directory, x, sep = "/"))$ctime) })
  ...
  out <- list(tl_report = tl_report, TileSchema = tile_schema, GridMoll = g_moll)
